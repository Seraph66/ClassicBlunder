...
/SecretInformation/HeavenlyRestriction/proc/applySecretVariable(mob/p, list/restr, improv)
	...
	switch(improv)
		   ...
		   if("Force")
			   p.passive_handler.Increase("Spirit Hand", min(boon_value, 4))
			   p.passive_handler.Increase("Spirit Sword", min(boon_value, 4))
			   p.passive_handler.Increase("Spirit Flow", min(boon_value, 4))
		   if("Endurance")
		   ...
		   if("Defense")
			   p.passive_handler.Increase("Fluid Form", min(boon_value, 4))
			   p.passive_handler.Increase("Like Water", min(boon_value, 4))
		   if("Speed")
