...
/SecretInformation/HeavenlyRestriction/proc/applySecretVariable(mob/p, list/restr, improv)
	...
	switch(improv)
		if("Dragon Clash")
			p.passive_handler.Increase("Technique Mastery", boon_value)
			p.passive_handler.Increase("Attack Speed", boon_value)
		if("Grab")
			p.passive_handler.Increase("Grippy", boon_value)
			p.passive_handler.Increase("Muscle Power", boon_value)
		if("Reverse Dash")
			// Already has passives, skip or add if needed
		if("Dragon Dash")
			p.passive_handler.Increase("Super Dash", 1)
			p.passive_handler.Increase("Skimming", boon_value)
		if("Strength")
			p.passive_handler.Increase("Momentum", boon_value)
			p.passive_handler.Increase("PhysPleroma", boon_value)
		if("Force")
			p.passive_handler.Increase("Spirit Hand", boon_value)
			p.passive_handler.Increase("Spirit Sword", boon_value)
			p.passive_handler.Increase("Spirit Flow", boon_value)
		if("Endurance")
			p.passive_handler.Increase("Juggernaut", boon_value)
			p.passive_handler.Increase("GiantForm", 1)
		if("Offense")
			p.passive_handler.Increase("Pressure", boon_value)
			p.passive_handler.Increase("ComboMaster", 1)
		if("Defense")
			p.passive_handler.Increase("Fluid Form", boon_value)
			p.passive_handler.Increase("Like Water", boon_value)
		if("Speed")
			p.passive_handler.Increase("Speed Force", 2)
			p.passive_handler.Increase("Pursuer", boon_value)
		if("Anger")
			p.passive_handler.Increase("Calm Anger", 1)
			p.passive_handler.Increase("Endless Anger", 1)
			p.passive_handler.Increase("Wrathful Tenacity", 0.5)
		if("Power Control")
			p.passive_handler.Increase("Energy Generation", boon_value * 1.5)
			p.passive_handler.Increase("Soft Style", boon_value * 0.5)
		if("Power")
			p.passive_handler.Increase("Buff Mastery", boon_value)
