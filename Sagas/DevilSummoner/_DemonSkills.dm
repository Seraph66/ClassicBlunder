...
/mob/Player/AI/Demon
	...
	proc/DemonSingleDamage(mob/target, dmg, element, debuff_val)
		...
		var/final_dmg = max(1, round(dmg * glob.DevilSummonerDemonSkillMod))
	...
	proc/DemonDanceSkill(mob/target, dmg_per, element, debuff_per, min_hits, max_hits)
		...
		var/final_dmg = max(1, round(dmg_per * glob.DevilSummonerDemonSkillMod))
		spawn()
	...
	proc/DemonDesperation(mob/target)
		...
		var/dmg = max(1, round(StrMod * 0.5 * (1 + missing_ratio * 3) * glob.DevilSummonerDemonSkillMod))
